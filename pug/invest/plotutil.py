...
def period_boxplot(df, period='year', column='Adj Close'):
    # df['period'] = df.groupby(lambda t: getattr(t, period)).aggregate(pd.np.mean)
...
def animate_panel(panel, keys=None, columns=None, interval=1000, titles='', path='animate_panel', xlabel='Time', ylabel='Value', **kwargs):
    ...
    >>> x = pd.np.arange(0, 2*pd.np.pi, 0.05)
    >>> panel = pd.Panel(dict((i, pd.DataFrame({
    ...        'T=10': pd.np.sin(x + i/10.),
    ...        'T=7': pd.np.sin(x + i/7.),
    ...        'beat': pd.np.sin(x + i/10.) + pd.np.sin(x + i/7.),
    ...        }, index=x)
    ...    ) for i in range(50)))
    ...
    def mask_lines():
        ...
        for i in range(len(lines)):
            ...
            lines[i].set_xdata(pd.np.ma.array(x, mask=True))
            lines[i].set_ydata(pd.np.ma.array(y[i], mask=True))
        return lines
